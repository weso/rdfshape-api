...
case class Outgoing(node: IRI, endpoint: IRI, children: Children) {
  def toJson: Json = Json.fromFields(
    List(
      ...
      ("children",
        Json.fromValues(
          children.m.map(pair => Json.fromFields(
            List(
              ("pred", Json.fromString(pair._1.toString)),
              ("values", Json.fromValues(pair._2.toList.map(value => Json.fromString(value.node.toString))))
            )
           )
          )
        )
...
object Outgoing {
  ...
  * Creates an outgoing value from a set of triples.
  ...
  def fromTriples(node: IRI, endpoint: IRI, ts: Set[RDFTriple]): Outgoing = {
    ...
    def cmb(m: Map[IRI,Vector[Value]], current: RDFTriple): Map[IRI,Vector[Value]] =
      m.get(current.pred).fold(
