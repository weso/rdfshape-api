...
case class ShapeMapInfoResult private ( msg: String,
  ...
  def toJson: Json = {
    Json.fromFields(List(("msg", Json.fromString(msg))) ++
      maybeField(shapeMap, "shapeMap", Json.fromString(_)) ++
      maybeField(shapeMapFormat, "shapeMapFormat", (sf: ShapeMapFormat) => Json.fromString(sf.name)) ++
...
object ShapeMapInfoResult {
  def fromMsg(msg: String): ShapeMapInfoResult = ShapeMapInfoResult(msg,None,None,None)
  def fromShapeMap(shapeMapStr: Option[String],shapeMapFormat: Option[ShapeMapFormat], shapeMap: ShapeMap): ShapeMapInfoResult =
    ShapeMapInfoResult("Well formed Shape Map",
      shapeMapStr,
      shapeMapFormat,
